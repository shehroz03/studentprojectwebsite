...
import { Users, ShoppingBag, DollarSign, CheckCircle, Eye, X, Download } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { supabase } from '../utils/supabaseClient';

// ─── Read user from localStorage ONCE outside component ──────────────────────
function readUser() {
  try {
...
export const AdminPanel = () => {
  ...
  // Guard: run fetch exactly once
  const hasFetched  = useRef(false);

  useEffect(() => {
    // Already fetched — do nothing (prevents double-call in React Strict Mode)
  ...
  const handleUpdateStatus = async (orderId, newStatus) => {
    try {
      ...
      let msg = '';
      if (newStatus === 'in-progress') msg = `Your order #${orderId.slice(0, 8)} is now In Progress!`;
      ...
      if (msg && order?.user_id) {
        await supabase.from('notifications').insert([{
          user_id: order.user_id, type: 'order_update', message: msg
        }]);
      }
    } catch (err) {
  ...
  const handleUpdatePaymentStatus = async (paymentId, newStatus) => {
    try {
      ...
      let msg = '';
      if (newStatus === 'verified') msg = `Your payment of ${payment?.currency || '$'}${payment?.amount} has been Verified!`;
      else if (newStatus === 'rejected') msg = `Your payment for order #${payment?.order_id?.slice(0, 8)} was Rejected.`;

      if (msg && payment?.user_id) {
        await supabase.from('notifications').insert([{
          user_id: payment.user_id, type: 'payment_update', message: msg
        }]);
      }
    } catch (err) {
  ...
  const handleDeliverAssignment = async (e) => {
    ...
    try {
      ...
      const filePath = `deliveries/delivery_${selectedOrder.id}_${Date.now()}.${fileExt}`;
      ...
      const { error: updateError } = await supabase
        .from('orders')
        .update({ status: 'completed', attachment_url: publicUrlData.publicUrl })
      ...
      if (selectedOrder?.user_id) {
        await supabase.from('notifications').insert([{
          ...
          message: `Your assignment for order #${selectedOrder.id.slice(0, 8)} has been delivered!`
        }]);
      }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {error && (
          <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-xl mb-6 text-sm">
            {error} — <button onClick={() => { hasFetched.current = false; window.location.reload(); }} className="underline ml-2">Retry</button>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          ...
          <StatCard icon={<DollarSign  className="text-cyan-400" />}    title="Verified Payments" value={payments.filter(p => p.status === 'verified').length} />
        </div>
        ...
        <div className="glass-card overflow-hidden">
          <div className="p-6 border-b border-white/10 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            {/* Filter */}
            <select
              className="bg-gray-800 border border-white/10 rounded-xl py-2 px-3 text-sm text-white focus:outline-none w-full md:w-auto cursor-pointer"
              value={filter}
                    ...
                    return (
                      <tr key={order.id} className="border-b border-white/5 hover:bg-white/5">
                        ...
                        <td className="px-6 py-5 text-white font-semibold">
                          {order.currency || '$'}{parseFloat(order.budget || 0).toLocaleString()}
                        </td>
                        <td className="px-6 py-5">
                          <select
              ...
              <table className="w-full text-left table-fixed">
                ...
                <tbody>
                  {filteredPayments.length === 0 ? (
                    <tr><td colSpan="5" className="p-10 text-center text-gray-500">No payments found.</td></tr>
            ...
            <Modal onClose={() => setSelectedOrder(null)} title="Order Details">
              <div className="space-y-4 text-white">
                <div>
                  ...
                  <div className="font-medium text-lg">{selectedOrder.title}</div>
                </div>
                <div>
                  <span className="text-gray-400 text-sm">Description:</span>
                  <div className="text-gray-300 bg-white/5 p-3 rounded-lg mt-1 text-sm whitespace-pre-wrap">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div><span className="text-gray-400 text-sm">Deadline:</span><div className="font-bold">{selectedOrder.deadline ? new Date(selectedOrder.deadline).toLocaleDateString() : '—'}</div></div>
                </div>
                ...
                <div className="pt-6 border-t border-white/10">
                  <h4 className="font-bold mb-4 flex items-center space-x-2">
                    <CheckCircle className="w-4 h-4 text-green-500" /><span>Deliver Final Assignment</span>
                  </h4>
            ...
            <Modal onClose={() => setSelectedPayment(null)} title="Payment Proof">
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div><span className="text-gray-400 text-sm">Status:</span><div className={`font-bold ${getStatusColor(selectedPayment.status)}`}>{selectedPayment.status?.toUpperCase()}</div></div>
                </div>
                {selectedPayment.proof_url ? (
                  <div className="pt-4 border-t border-white/10">
                    <p className="text-gray-400 text-xs mb-2">Payment Screenshot:</p>
                    <a href={selectedPayment.proof_url} target="_blank" rel="noreferrer"
                      className="block rounded-xl overflow-hidden border border-white/10">
                      <img src={selectedPayment.proof_url} alt="Payment Proof"
                        className="w-full h-auto max-h-[400px] object-contain bg-black/50" />
                    </a>
                  </div>
                ) : (
                  <div className="text-gray-500 text-sm text-center py-8">No payment screenshot uploaded.</div>
                )}
              </div>
            </Modal>
          )}
...
const Modal = ({ title, children, onClose }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <motion.div
      ...
      className="relative bg-gray-900 border border-white/10 shadow-2xl rounded-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
